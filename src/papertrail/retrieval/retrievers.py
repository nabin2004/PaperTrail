...
import time
import logfire
from papertrail.utils.observability import retrieval_histogram
...
class PaperRetriever:
    ...
    def retrieve(self, query: str, k: int = 10) -> List[SearchResult]:
        ...
        start_t = time.time()
        with logfire.span("retrieval.retrieve", query=query, top_k=k):
            ...
            retrieval_histogram.record(duration, {"top_k": str(k)})
            logfire.info(
                "Retrieved {count} chunks for query '{query}' in {duration:.3f}s",
                count=len(results),
                query=query[:60],
                duration=duration,
            )
